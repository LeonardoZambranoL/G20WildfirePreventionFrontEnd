...
export default function PossibleMigration() {
  const srcs = ["https://de.wikipedia.org/wiki/Horse"];
  const buttonDescriptions: string[] = [];
  const descriptions = [`${lorem}`];
  return (
    <>
      <GraphOnSide
        srcs={srcs}
        title="Migration patterns tentatively due to wildfires"
